...
public class DriverTracking extends AppCompatActivity implements OnMapReadyCallback , GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener{
    ...
    double riderLat, riderLng, destLat, destLng;
    ...
    FloatingActionButton googleMapsFAB;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(getIntent()!=null){
            ...
            destLat=getIntent().getDoubleExtra("lat_end",-1.0);
            destLng=getIntent().getDoubleExtra("lng_end",-1.0);
        ...
        // The boolean that decides whether the trip has started or not
        isTripStarted = false;

        googleMapsFAB = (FloatingActionButton)findViewById(R.id.googleMapFAB);
        googleMapsFAB.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Log.d("DEBUG", "Track on google maps");
                // Create a Uri from an intent string. Use the result to create an Intent.

                LatLng destinationNow;

                if (isTripStarted == false) {
                    destinationNow = new LatLng(riderLat, riderLng);
                } else {
                    destinationNow = new LatLng(destLat, destLng);
                }
                Uri gmmIntentUri = Uri.parse("http://maps.google.com/maps?saddr="
                        + Common.currentLat.toString() + "," + Common.currentLng.toString() +
                        "&daddr=" +
                        destinationNow.latitude + "," + destinationNow.longitude);
    ...
    private void calculateCashFree(final LatLng pickupLocation, LatLng latLng) {
        ...
        try {
            requestApi = "https://maps.googleapis.com/maps/api/directions/json?" +
                    ...
                    "key=" + "AIzaSyCW328LblonACBGdYhQuyw16TGNmtOph20";
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        // Used to get the location of the driver if he is around
        geoQuery.addGeoQueryEventListener(new GeoQueryEventListener() {
            @Override
            public void onKeyEntered(String key, GeoLocation location) {
                sendArrivedNotification(riderToken);
                // Enables the startTrip button if driver arrives
    ...
    private class ParserTask extends AsyncTask<String, Integer, List<List<HashMap<String, String>>>> {
        ...
        protected void onPostExecute(List<List<HashMap<String, String>>> lists) {
            ...
            Log.d("DEBUG", "Lists : " + lists.toString());

            // Commented for Google API Limit purposes
            try{
            ...
            // Google Directions API Issue
            // The polylineOptions is null (Fix It)
